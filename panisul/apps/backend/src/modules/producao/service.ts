...
export async function processProduction(input: CreateProductionInput) {
	return await prisma.$transaction(async (tx) => {
		...
		for (const out of input.outputs) {
			...
			await tx.stockMove.create({ data: { productId: out.productId, quantity: Math.abs(out.quantity), reason: "PRODUCAO" } as any });
		}
		for (const cons of input.consumption ?? []) {
			await tx.product.update({ where: { id: cons.productId }, data: { stockQty: { decrement: cons.quantity } } });
			await tx.stockMove.create({ data: { productId: cons.productId, quantity: -Math.abs(cons.quantity), reason: "PRODUCAO" } as any });
		}
