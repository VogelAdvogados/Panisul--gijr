...
clientesRouter.get("/:id/receivables", authMiddleware, requireRoles("ADMIN", "VENDEDOR"), async (req, res, next) => {
	try {
		...
		return res.status(200).json(makeResponse(list.map((r) => ({ 
			id: r.id, 
			dueDate: r.dueDate.toISOString(), 
			amount: r.amount, 
			status: r.status, 
			saleId: r.saleId 
		})), { message: "Recebíveis", traceId: req.traceId }));
	} catch (err) {
		return next(err);
	}
});
...
clientesRouter.get("/:id/sales", authMiddleware, requireRoles("ADMIN", "VENDEDOR"), async (req, res, next) => {
	try {
		...
		return res.status(200).json(makeResponse(sales.map((s) => ({ 
			id: s.id, 
			createdAt: s.createdAt.toISOString(), 
			totalValue: s.totalValue, 
			paymentType: s.paymentType 
		})), { message: "Histórico de compras", traceId: req.traceId }));
	} catch (err) {
		return next(err);
	}
});
...
clientesRouter.get("/:id/exchanges", authMiddleware, requireRoles("ADMIN", "VENDEDOR"), async (req, res, next) => {
	try {
		...
		return res.status(200).json(makeResponse(exchanges.map((e) => ({ 
			id: e.id, 
			createdAt: e.createdAt.toISOString(), 
			items: e.items 
		})), { message: "Histórico de trocas", traceId: req.traceId }));
	} catch (err) {
		return next(err);
	}
});